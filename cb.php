 ...
 */
class CBAPI {
	private static $APIKey = "WDT90KG79371981LL9F4";  /* your private CareerBuilder API Developer Key */
	...
	/*
	 *  FUNCTION:     getJobResultsCompany
	 *  DESCRIPTION:  Function to run a job search (based on Company) on careerbuilder and return back an array of the
	 *                results that matched the search.
	 *  INPUT:        (organization:string) string for the keywords of the search
	 *  INPUT:        (location:string) string for the location of the search
	 *  INPUT:        (country:string) country code in which to search
	 *  INPUT:        (pagenumber:number) page of results to retrieve, start at page 0
	 *  INPUT:        (daysBackToLook:number:optional) values from 1-30, used to add a time constraint
	 ...
	 *
	 */	
	public static function getJobResultsCompany($organization, $options=array())
	{
	  $defaults = array(
	                    'location'=>NULL,
	                    'country'=>NULL,
  	...
  	$options = array_merge($defaults,$options);
		...
		$url = 'http://api.careerbuilder.com/v1/jobsearch?DeveloperKey='.$key.
		       '&ExcludeNational=True&siteid='.$siteIDValue.
		       '&CompanyName='.$organization.'&PerPage='.$perPage.
		...
		if($options['daysBackToLook'] != NULL)
		{
			$url .= '&PostedWithin='.$options['daysBackToLook'];
		}
		
		if($options['country'] != '') {
			$url = $url . '&CountryCode='.$options['country'];
		}
		
		$xml = simplexml_load_file($url);
		//echo highlight_string(print_r($xml,true),true);exit;
		$jobsCollection = Array();
		$currItem = 0;
		foreach($xml->Results->JobSearchResult as $result)
		{
	    ...
	    $currJob->title = (string)$result->JobTitle;
	    $currJob->companyName = (string)$result->Company;
	    $currJob->location = (string)$result->Location;
	    $currJob->city = (string)$result->LocationCity;
	    $currJob->state = (string)$result->LocationState;
	    $currJob->url = (string)$result->JobDetailsURL;
	    $currJob->latitude = (string)$result->LocationLatitude;
	    $currJob->longitude = (string)$result->LocationLongitude;
	    $currJob->description = (string)$result->DescriptionTeaser; 
	    $currJob->posted = (string)$result->PostedDate;
	    $currJob->logo = (string)$result->CompanyImageURL;
			$jobsCollection[$currItem] = $currJob;
			$currItem ++;
		}
		return $jobsCollection;
